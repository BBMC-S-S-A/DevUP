 ...
 * servidores levantados. No hay dobles de prueba a propósito.
 *
 * PORTADO desde `claude/inicio-desarrollo-nu1ftu` el 5 de septiembre de 2026.
 * Playwright sigue sin ser dependencia del proyecto —es una decisión pendiente
 * y anotada en `docs/traspaso-2026-08-29.md`, no tomada aquí—, así que se
 * instala aparte y no vive en `package.json`:
 *
 *   npm run dev                       # en otra terminal
 *   npm install --no-save playwright
 *   npm run test:e2e
 *
 * NO EJECUTADO EN ESTE CAMBIO: se escribió y se adaptó sin Docker a mano, sin
 * poder correr `npm run dev` de verdad. Antes de fiarse de esta suite hace
 * falta una pasada en una máquina con Docker.
 */
