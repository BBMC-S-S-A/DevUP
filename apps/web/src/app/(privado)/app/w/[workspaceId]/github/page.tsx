  ...
  const load = useCallback(async () => {
    try {
      const [{ connections }, { repos }] = await Promise.all([
        api.get<{ connections: Connection[] }>(`/workspaces/${workspaceId}/connections`),
        api.get<{ repos: GithubRepo[] }>(`/workspaces/${workspaceId}/github/repos`),
      ]);
 ...
 */
function NuevoRepo({ workspaceId, onAdded }: { workspaceId: string; onAdded: (repo: GithubRepo) => void }) {
