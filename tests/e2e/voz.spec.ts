 ...
 * de `ayudantes.ts`. Cada texto de `VoiceRoom.tsx` se contrastó con el código
 * actual («Sala de voz», «en directo», «Grabar», «Acepto que se grabe»,
 * «Petición para grabar», «Esperando permiso», «Detener», «Colgar», «No»,
 * «no aceptó que se grabara» — todos confirmados). La única excepción es «Se
 * ha guardado la grabación»: no se encontró ese texto en ningún componente ni
 * en el hook `useVoiceRoom`, así que en vez de adivinarlo se quitó esa
 * aserción — la comprobación real que sigue justo debajo, que el archivo
 * aparece en la biblioteca al recargar, no depende de ninguna redacción.
 */
test.describe("Voz", () => {
  ...
  test("nadie graba sin que todos lo acepten", async ({ browser }) => {
    ...
    // esta comprobación no depende de ninguna redacción de toast, solo de que
    // el archivo exista al volver a la biblioteca.
