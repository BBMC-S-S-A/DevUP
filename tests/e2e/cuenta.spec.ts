 ...
 * de `ayudantes.ts` para el porqué. Y una segunda pasada, tras rebasar sobre
 * 62 commits nuevos en la base: invitar ganó un enlace copiable y un
 * `workspaceId` opcional (`account.ts`), lo que cambió el texto del aviso y
 * cómo se pinta cada invitación pendiente. Ajustes verificados contra el
 * código actual:
 *   - La pantalla de «invitación ya usada» dice «Ya se usó», no «Esta
 *     invitación ya se usó.» (`app/invitacion/page.tsx`).
 *   - El rol de quien invita ya no va en la misma frase que el nombre — hoy es
 *     un chip aparte («Miembro», «Admin»…), así que se comprueba solo la
 *     frase que sigue siendo cierta: quién invitó, antes de pedir nada.
 *   - El aviso de invitar dice «Invitación creada para…», no «Invitación
 *     enviada a…» — ahora hay un enlace copiable de por medio, y el texto lo
 *     dice.
 *   - En la lista de pendientes, el correo y el rol son dos nodos separados
 *     (un `<span>` y un `<Chip>`), no una frase «correo · rol» — se localiza
 *     la fila por el correo y se comprueba que también lleva el rol.
 */
test.describe("Cuenta e invitaciones", () => {
  ...
  test("ciclo completo de invitación: enviar, abrir, aceptar y entrar", async ({ browser }) => {
    ...
    await expect(page.getByText(new RegExp(`Invitación creada para ${invitada}`))).toBeVisible();
    // El correo y el rol ya no van en una sola frase «email · rol»: ahora son
    // dos nodos —el correo en un `<span>`, el rol en un `<Chip>`— dentro de la
    // misma fila. Se localiza la fila por el correo y se comprueba que
    // también lleva el rol, en vez de buscar una frase que ya no existe.
    const filaInvitacion = page.locator("li").filter({ hasText: invitada });
    await expect(filaInvitacion).toContainText("member");
    ...
    await expect(otra.getByText(/Ana Prueba te ha invitado/)).toBeVisible();
  ...
  test("una invitación gastada no sirve dos veces", async ({ browser }) => {
    ...
    await expect(segunda.getByText("Ya se usó")).toBeVisible();
